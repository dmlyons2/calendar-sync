...
from calendar_sync.models import Create, Delete, SourceEvent, TargetEvent, Update
...
def _src(uid="uid-1", sequence=1) -> SourceEvent:
    return SourceEvent(
        ...
        end=datetime(2026, 6, 1, 16, 0, tzinfo=timezone.utc),
        tzid="UTC",
        rrule=None,
        exdates=(),
        status="CONFIRMED",
...
def test_apply_actions_calls_client_methods():
    ...
    assert result == SyncResult(created=1, updated=1, deleted_cancelled=1, deleted_vanished=1, errors=0)
...
def test_apply_actions_dry_run_makes_no_calls():
    ...
    assert result == SyncResult(created=1, updated=0, deleted_cancelled=1, deleted_vanished=0, errors=0)
